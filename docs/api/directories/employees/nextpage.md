...
- **Method:** `POST`
- **Route:** `/v1/directories/employees/list/nextpage`
...
Returns the same `EmployeeRowApiResponse` format as the initial `/list` call.

## Usage example

```bash
curl -X POST "https://api.armsoft.am/accountant/v1/directories/employees/list/nextpage" \
