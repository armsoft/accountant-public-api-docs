...
- **Method:** `POST`
- **Route:** `/v1/directories/partners/list/nextpage`
...
Returns the same `PartnerRowApiResponse` format as the initial `/list` call.

## Usage example

```bash
curl -X POST "https://api.armsoft.am/accountant/v1/directories/partners/list/nextpage" \
