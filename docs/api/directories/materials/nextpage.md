...
- **Method:** `POST`
- **Route:** `/v1/directories/materials/list/nextpage`
...
Returns the same `MaterialRowApiResponse` format as the initial `/list` call.

## Usage example

```bash
curl -X POST "https://api.armsoft.am/accountant/v1/directories/materials/list/nextpage" \
